...
function Collection({ title, blank, fields }) {
  const { user } = useAuth();
  ...
  const load = () => setItems(getClients(user?.uid));
  useEffect(() => { if (user?.uid) load(); }, [user?.uid]);

  const save = (e) => {
    e.preventDefault();
    if (!form.name) return toast.error('Name is required.');
    saveClient(user.uid, form);
    load();
    setForm(null);
  ...
  const remove = async (id) => {
    const confirmed = await confirm(`Delete this ${title.slice(0, -1).toLowerCase()}? This cannot be undone.`, {
      title: 'Delete',
      confirmLabel: 'Delete',
      danger: true,
    });
  ...
  const shown = items.filter(x => Object.values(x).join(' ').toLowerCase().includes(query.toLowerCase()));

  return (
    <ProtectedLayout title={title} subtitle={`Create, edit, search, and track ${title.toLowerCase()}.`}>
      <div className="glass-card panel-card">
        <div className="card-head">
          ...
          <button className="btn btn-primary" onClick={() => setForm(blank)}>Add {title.slice(0, -1)}</button>
        </div>
        ...
        <div className="table-shell">
          <table>
            <thead>
              <tr>
                {fields.slice(0, 4).map(f => <th key={f[0]}>{f[1]}</th>)}
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {shown.map(x => (
                <tr key={x.id}>
                  {fields.slice(0, 4).map(f => <td key={f[0]}>{x[f[0]]}</td>)}
                  <td>
              ...
              {!shown.length && <tr><td colSpan="5">No {title.toLowerCase()} yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
                ...
                <label key={name}>
                  ...
                  {name === 'status' ? (
                    <select name={name} value={form[name]} onChange={e => setForm({ ...form, [name]: e.target.value })}>
                      {['New', 'Contacted', 'Viewing Scheduled', 'Offer Made', 'Closed'].map(x => <option key={x}>{x}</option>)}
                    </select>
                  ) : name === 'notes' ? (
                    <textarea name={name} value={form[name]} onChange={e => setForm({ ...form, [name]: e.target.value })} />
                  ) : (
                    <input type={inputType || 'text'} name={name} value={form[name]} onChange={e => setForm({ ...form, [name]: e.target.value })} />
                  )}
                </label>
              ))}
            </div>
...
export function Users() {
  ...
  const remove = async (u) => {
    const confirmed = await confirm(`Delete ${u.name || u.email}? This cannot be undone.`, {
      title: 'Delete User',
      confirmLabel: 'Delete',
      danger: true,
    });
